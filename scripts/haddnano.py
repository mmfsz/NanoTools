...
# Script adapted from https://raw.githubusercontent.com/cms-nanoAOD/nanoAOD-tools/master/scripts/haddnano.py
...
# get file list checking if regex "*" in file name
allfiles = []
for fin in files:
    if "*" in fin:
        allfiles += [f for f in glob.glob(fin)] 
    else:
        allfiles += [fin]
files = allfiles
