...
public:
  ...
  // Constructor
  Analysis_1Lepton_1FJ(Arbusto &arbusto_ref, Nano &nt_ref, HEPCLI &cli_ref, Cutflow &cutflow_ref)
  ...
  void initCutflow() override
  {
    ...
    // Lepton selection
    Cut *cut_onelepton = new LambdaCut(
        "OneLepton",
        [&]()
        {
          return (cutflow.globals.getVal<LorentzVectors>("vvh_veto_lep_p4s").size() >= 1);
        });
    ...
    Cut *cut_atleast1fatjet = new LambdaCut(
        "AtLeast1FatJet",
        [&]()
        {
          return (cutflow.globals.getVal<int>("n_vvh_veto_fatjets") >= 1);
        });
