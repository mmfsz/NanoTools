...
public:
  ...
  // Constructor
  Analysis_0Leptons_3FJ(Arbusto &arbusto_ref, Nano &nt_ref, HEPCLI &cli_ref, Cutflow &cutflow_ref)
  ...
  void initCutflow() override
  {
    ...
    // Lepton selection
    Cut *cut_atleast3fatjets = new LambdaCut(
        "AtLeast3FatJets",
        [&]()
        {
          return (cutflow.globals.getVal<int>("n_vvh_veto_fatjets") >= 3);
        });
